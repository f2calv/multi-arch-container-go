...
docker run --rm \
  ...
  -e OTEL_SERVICE_NAME=multi-arch-container-go \
  -e OTEL_RESOURCE_ATTRIBUTES=deployment.environment.name=development \
  ghcr.io/f2calv/multi-arch-container-go
```

The official exporters also honor signal-specific `OTEL_EXPORTER_OTLP_*` variables for endpoints, headers, compression, certificates and timeouts. When the base endpoint is absent, no OpenTelemetry SDK or exporter is initialized.
